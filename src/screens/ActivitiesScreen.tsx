...
// ─── Category icon & color ────────────────────────────────────────────────────
const CAT_META: Record<ActivityCategory, { icon: string; color: string; label: string }> = {
  mind:      { icon: '🧠', color: COLORS.sapphire, label: 'Mind'      },
  body:      { icon: '💪', color: COLORS.teal,     label: 'Body'      },
  social:    { icon: '👥', color: COLORS.orchid,   label: 'Social'    },
  financial: { icon: '💰', color: COLORS.gold,     label: 'Financial' },
  illegal:   { icon: '💀', color: COLORS.crimson,  label: 'Illegal'   },
  health:    { icon: '❤️', color: COLORS.crimson,  label: 'Health'    },
  misc:      { icon: '✨', color: COLORS.gold3,    label: 'Misc'      },
};
      ...
      <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterBar} contentContainerStyle={styles.filterContent}>
          {ALL_CATS.map(cat => {
            ...
            const label = cat === 'all' ? 'All' : CAT_META[cat as ActivityCategory].label;
            return (
              <Pressable
                key={cat}
              ...
              >
                {cat !== 'all' && <Text style={styles.filterIcon}>{CAT_META[cat as ActivityCategory].icon}</Text>}
                <Text style={[styles.filterLabel, active && { color: COLORS.gold }]}>{label}</Text>
              </Pressable>
            );
          })}
        </ScrollView>
