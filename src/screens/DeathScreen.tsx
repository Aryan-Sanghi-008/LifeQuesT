...
function Tombstone() {
  return (
    <Svg width={160} height={200} viewBox="0 0 160 200">
      ...
      {/* Base slab */}
      <SvgRect x={20} y={165} width={120} height={18} rx={4} fill="url(#stoneGrad)" />
      <SvgRect x={10} y={178} width={140} height={10} rx={3} fill="url(#stoneGrad)" />
      ...
      {/* Cross ornament */}
      <SvgRect x={76} y={44} width={8} height={32} rx={2} fill="url(#goldGrad)" />
      <SvgRect x={62} y={54} width={36} height={8} rx={2} fill="url(#goldGrad)" />

      {/* Name plate */}
      <SvgRect x={38} y={95} width={84} height={48} rx={6} fill="rgba(0,0,0,0.3)" />
      <SvgRect x={39} y={96} width={82} height={46} rx={5} fill="none" stroke={`${COLORS.gold}25`} strokeWidth={1} />

      {/* R.I.P text */}
      <Path d="M55 108 h50" stroke={COLORS.gold3} strokeWidth={1} strokeLinecap="round" opacity={0.5} />
...
// ─── Floating Particle ────────────────────────────────────────────────────────
function FloatingParticle({ x, delay, color }: { x: number; delay: number; color: string }) {
  ...
  useEffect(() => {
    const animate = () => {
      ...
      opacity.setValue(0);
      Animated.parallel([
        Animated.timing(y, { toValue: height * 0.1 - Math.random() * 100, duration: 4000 + Math.random() * 3000, useNativeDriver: true }),
        Animated.sequence([
          Animated.timing(opacity, { toValue: 0.6, duration: 800, useNativeDriver: true }),
          Animated.timing(opacity, { toValue: 0, duration: 1200, delay: 2000, useNativeDriver: true }),
        ]),
  ...
  return (
    <Animated.View
      style={{
        position: 'absolute', left: x, width: 4, height: 4,
...
// ─── Stat Row ─────────────────────────────────────────────────────────────────
function FinalStatRow({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <View style={fsr.row}>
...
const fsr = StyleSheet.create({
  ...
  label:  { fontFamily: FONTS.bodySemiBold, fontSize: 11, color: COLORS.t3, width: 80, textAlign: 'right' },
  barWrap:{ flex: 1 },
  val:    { fontFamily: FONTS.monoSemiBold, fontSize: 12, width: 28, textAlign: 'right' },
});
...
export default function DeathScreen() {
  ...
  useEffect(() => {
    ...
    Animated.stagger(300, [
      ...
      Animated.timing(tombAnim,   { toValue: 1, duration: 700, useNativeDriver: true }),
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(tombFloat, { toValue: -8, duration: 2500, useNativeDriver: true }),
        Animated.timing(tombFloat, { toValue: 0,  duration: 2500, useNativeDriver: true }),
      ])
    ).start();
  }, []);
  ...
  const { name, age, deathAge, deathCause, stats, karma,
  ...
  const handleShare = async () => {
    const message = [
      `${name} lived ${finalAge} years (${birthYear}–${birthYear + finalAge}).`,
      `Rating: ${lifeRating}`,
      `Died of ${deathCause ?? 'natural causes'}.`,
      `Achievements: ${achievements.length} · Events: ${eventHistory.length}`,
      `Karma: ${karma}`,
    ].join('\n');
        ...
        >
          ...
          <Animated.View style={[styles.header, { opacity: headerAnim, transform: [{ translateY: headerAnim.interpolate({ inputRange: [0, 1], outputRange: [-20, 0] }) }] }]}>
            <View style={styles.ribbonWrap}>
              <LinearGradient colors={[COLORS.bg3, `${COLORS.crimson}18`, COLORS.bg3]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.ribbon}>
          ...
          <Animated.View style={[styles.tombWrap, {
            opacity: tombAnim,
            transform: [
              { translateY: Animated.add(tombAnim.interpolate({ inputRange: [0, 1], outputRange: [30, 0] }), tombFloat) },
            ],
          }]}>
            <Tombstone />
            ...
            <View style={styles.tombText}>
              ...
              <Text style={styles.tombDates}>{birthYear} — {birthYear + finalAge}</Text>
              <Text style={styles.tombCause}>Died of {deathCause ?? 'natural causes'}</Text>
            </View>
          ...
          <Animated.View style={[{ opacity: cardAnim, transform: [{ translateY: cardAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }]}>
            ...
            <Card style={styles.statsCard}>
              ...
              <FinalStatRow label="Health"     value={stats.health}       color={COLORS.crimson}  />
              <FinalStatRow label="Happiness"  value={stats.happiness}    color={COLORS.gold}     />
              <FinalStatRow label="Mind"       value={stats.intelligence} color={COLORS.sapphire} />
              <FinalStatRow label="Wealth"     value={stats.wealth}       color={COLORS.teal}     />
              <FinalStatRow label="Fitness"    value={stats.fitness}      color={COLORS.emerald}  />
              <FinalStatRow label="Looks"      value={stats.looks}        color={COLORS.orchid}   />
              <FinalStatRow label="Social"     value={stats.social}       color={COLORS.sapphire} />
              <FinalStatRow label="Ambition"   value={stats.ambition}     color={COLORS.gold}     />
            </Card>
            ...
            <Card style={styles.highlightsCard}>
              ...
              <View style={styles.highlights}>
                {[
                  { label: 'Events Lived', value: eventHistory.length, color: COLORS.sapphire },
                  { label: 'Relationships', value: relationships, color: COLORS.crimson },
              ...
              <Card style={styles.achCard}>
                ...
                <View style={styles.achRow}>
                  {achievements.map(id => {
                    ...
                    if (!ach) return null;
                    return (
                      <View key={id} style={[styles.achChip, { borderColor: `${ach.color}40`, backgroundColor: `${ach.color}10` }]}>
                        <Svg width={10} height={10} viewBox="0 0 24 24" fill={ach.color}>
                          <Path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                        </Svg>
                        <Text style={[styles.achChipText, { color: ach.color }]}>{ach.label}</Text>
                      </View>
                    );
                  })}
                </View>
          ...
          <Animated.View style={[styles.cta, { opacity: ctaAnim }]}>
            ...
            <Pressable style={styles.shareBtn} onPress={handleShare}>
              <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
                ...
                <Circle stroke={COLORS.t3} strokeWidth={2} cx="6" cy="12" r="3"/>
                <Circle stroke={COLORS.t3} strokeWidth={2} cx="18" cy="19" r="3"/>
                <Line stroke={COLORS.t3} strokeWidth={2} strokeLinecap="round" x1="8.59" y1="13.51" x2="15.42" y2="17.49"/>
                <Line stroke={COLORS.t3} strokeWidth={2} strokeLinecap="round" x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>
              </Svg>
              <Text style={styles.shareBtnText}>Share Life Story</Text>
            </Pressable>
          </Animated.View>
...
const styles = StyleSheet.create({
  ...
  // Header ribbon
  header:     { width: '100%', alignItems: 'center', marginTop: SPACING.xl, marginBottom: SPACING.xl },
  ...
  tombDates:{ fontFamily: FONTS.mono, fontSize: 9, color: COLORS.t3, marginTop: 3 },
  tombCause:{ fontFamily: FONTS.body, fontSize: 9, color: COLORS.t4, marginTop: 4, textAlign: 'center' },
  tombGlow: {
    position: 'absolute', bottom: -10, alignSelf: 'center',
    width: 120, height: 20, borderRadius: 60,
    backgroundColor: COLORS.gold, opacity: 0.07,
    ...SHADOWS.gold,
  },

  // Rating
  ratingRow:   { flexDirection: 'row', alignItems: 'center', gap: SPACING.xl, marginBottom: SPACING.lg, width: '100%' },
  ratingBadge: { alignItems: 'center', justifyContent: 'center', width: 80, height: 80, borderRadius: 40, borderWidth: 2, borderColor: COLORS.goldBorder, backgroundColor: `${COLORS.gold}10` },
  ratingAge:   { fontFamily: FONTS.displayBlack, fontSize: 28, color: COLORS.gold },
  ratingYrs:   { fontFamily: FONTS.body, fontSize: 10, color: COLORS.gold3 },
  ratingLabel: { fontFamily: FONTS.displayBold, fontSize: 24, color: COLORS.t1 },
  ratingCountry: { fontFamily: FONTS.body, fontSize: 13, color: COLORS.t3, marginTop: 4 },
  ...
  cardTitle:      { fontFamily: FONTS.bodySemiBold, fontSize: 10, color: COLORS.t4, letterSpacing: 2, marginBottom: SPACING.sm },

  // Highlights grid
  highlights:    { flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.sm },
  highlightItem: { width: '30%', alignItems: 'center', paddingVertical: SPACING.sm },
  highlightVal:  { fontFamily: FONTS.displayBold, fontSize: 20 },
  highlightLbl:  { fontFamily: FONTS.body, fontSize: 10, color: COLORS.t4, marginTop: 2, textAlign: 'center' },
