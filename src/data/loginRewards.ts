...
function buildLoginRewardSchedule(): LoginReward[] {
  ...
  for (let d = 1; d <= 30; d++) {
    if (d <= 6) {
      const coins = 500 * d;
      schedule.push({ day: d, coins, label: `${coins.toLocaleString()} Coins` });
    } else if (d === 7) {
      schedule.push({ day: d, gems: 5, label: '5 Gems' });
    } else if (d <= 13) {
      const coins = 2200 + (d - 8) * 300;
      schedule.push({ day: d, coins, luckBoost: 3, label: `${coins.toLocaleString()} Coins + Luck Boost` });
    } else if (d === 14) {
      schedule.push({ day: d, avatarStyleUnlock: 'lorelei', label: 'Rare Avatar: Lorelei' });
    } else if (d <= 20) {
      const coins = 4000 + (d - 15) * 300;
      schedule.push({ day: d, coins, seasonXp: 100, label: `${coins.toLocaleString()} Coins + 100 XP` });
    } else if (d === 21) {
      schedule.push({ day: d, mysteryBoxSpin: true, label: 'Bonus Mystery Box Spin' });
    } else if (d <= 27) {
      const coins = 5500 + (d - 22) * 250;
      schedule.push({ day: d, coins, mysteryTickets: 1, label: `${coins.toLocaleString()} Coins + Mystery Ticket` });
    } else if (d === 28) {
      ...
      schedule.push({
        ...
        label: 'Legendary Cosmetic + 10 Gems',
      });
    }
  }
  return schedule;
